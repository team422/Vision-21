...
//no idea what this is: import jdk.jfr.internal.PrivateAccess;
...
public static class ShirtPipeline implements VisionPipeline {
  ...
  private static Mat drawnFrame = new Mat(120, 160, 8);
  ...
  public static void main(String... args) {
    ...
    if (cameras.size() >= 1) {
      // CvSource drawnVideo = new CvSource("Shirt Detector", cameras.get(0).getVideoMode());
      CameraServer inst = CameraServer.getInstance();
      CvSource drawnVideo = inst.putVideo("Vision Server", 160, 120);
      // cameras.add(drawnVideo);
      // MjpegServer visionServer = drawnVideo;
      ...
      new ShirtPipeline(), pipeline -> {
        ...
        if(ShirtPipeline.findContoursOutput.size()>0){
          ...
          System.out.println("Contour " + maxSizeIndex + " was the largest with a size of " + maxSize + ".");
          Rect boundRect = Imgproc.boundingRect(ShirtPipeline.findContoursOutput.get(maxSizeIndex));
          double centerX = boundRect.x + (boundRect.width / 2);
          double centerY = boundRect.y + (boundRect.height / 2);
          System.out.println("The centerpoint of the largest contour is (" + centerX + "," + centerY + ")");
          ...
          //System.out.println("Use mat type " + ShirtPipeline.blurOutput.type());
